...
\begin{itemize}
  ...
  \item \textbf{\texttt{pdf/Algebra I.pdf}}：用于把 Galois、模论、环论、表示论四大主块按课程讲义顺序快速对齐，可直接对应本提纲第二至第五阶段。
...
\begin{itemize}
  ...
  \item \textbf{\texttt{pdf/Algebra III.pdf}}：用于补交换代数背景，尤其是 Noether 正规化、局部化、flatness、Noether/Artin、维数、Koszul/regular sequence 等内容；主要对应本提纲第二、第三、第六阶段的背景对标。
\end{itemize}
...
\paragraph{教材对标（Algebra I / III）}
\begin{itemize}
  \item \textbf{Algebra I}：先扫目录级主线即可，即 Chapter 1（Fields and Galois Theory）、Chapter 2（Module Theory）、Chapter 3（Ring Theory）、Chapter 4（Representation Theory），明确后面每一阶段各自回到哪一章。
  \item \textbf{Algebra III}：先记住 Chapter 1--10 的大轮廓，尤其是 1.2（Nakayama）、2（Localization）、3（Flatness）、4.3（Noether normalization）、5（Noetherian/Artinian）、8--10（completion、dimension、regular/CM/normal）；这一轮只做定位，不展开细节。
\end{itemize}
...
\paragraph{教材对标（Algebra I / III）}
\begin{itemize}
  \item \textbf{Algebra I}：对应 Chapter 1 全章，重点是 1.5（Galois extensions）、1.6（Galois correspondence）、1.7（Galois groups of polynomials）、1.10（Finite fields）、1.11--1.14（cyclotomic / solvability / constructions）、1.15--1.20（infinite Galois、norm/trace、Kummer、Artin--Schreier、transcendence）。
  \item \textbf{Algebra III}：只对标其中与考纲直接相接的交换代数背景，优先看 4.3（Noether normalization and Nullstellensatz）与 9.3（transcendental dimension）；它们不是本阶段主刷题源，只用于补“超越扩张”和“Noether 正规化”的语言背景。
\end{itemize}
...
\paragraph{教材对标（Algebra I / III）}
\begin{itemize}
  \item \textbf{Algebra I}：对应 Chapter 2 全章，重点是 2.1--2.4（modules / free / finitely generated / short exact sequences）、2.5--2.8（chain conditions、Schur、Jordan--Holder）、2.9--2.11（semisimple / indecomposable / Krull--Schmidt）、2.12--2.13（tensor product）。
  \item \textbf{Algebra III}：可对照 1.2（Nakayama lemma）、2.2（Localization of modules）、3.1（Flat modules）、5.1--5.3（Noetherian / Artinian）来补“有限生成”“链条件”“正合性局部检验”这类交换代数视角，但仍以 Algebra I 为主。
\end{itemize}
...
\paragraph{教材对标（Algebra I / III）}
\begin{itemize}
  \item \textbf{Algebra I}：对应 Chapter 3 全章，重点是 3.1--3.2（semisimple/simple rings）、3.3--3.5（Jacobson radical、nil ideals、semiprimitive / semiprimary）、3.6--3.7（density theorem、primitive rings）、3.8（central simple algebras）、3.9（Brauer groups）。
  \item \textbf{Algebra III}：这里只作概念侧面对照，尤其是 1.2 中 Jacobson radical/局部环语言，帮助区分“交换代数中的 Jacobson 根”与“非交换半单理论中的 Jacobson 根”各自承担的角色，不额外扩展复习负担。
\end{itemize}
...
\paragraph{教材对标（Algebra I / III）}
\begin{itemize}
  \item \textbf{Algebra I}：对应 Chapter 4 全章，重点是 4.1--4.4（representations、group algebra、character、基本例子）、4.5--4.6（induced representations、Frobenius reciprocity、Clifford）、4.7--4.10（Brauer induction、rationality 等提高内容）、4.11（symmetric groups）。
  \item \textbf{Algebra III}：这一阶段基本不需要额外展开；若要补背景，只需记住群代数半单性与前面环论阶段、模论阶段是连着的，不必另开交换代数支线。
\end{itemize}
...
\paragraph{教材对标（Algebra I / III）}
\begin{itemize}
  \item \textbf{Algebra I}：本阶段没有整章正面对标，但 Chapter 2 的 exact sequence、tensor product 与 semisimple/projective 的直觉应当带着进入同调代数。
  \item \textbf{Algebra III}：这里是最有用的辅助对标来源，优先回看 1.2（Nakayama）、2.2（Localization of modules）、3.1--3.3（flatness 与 going-down）、8.3（local criterion of flatness）、10.2（regular sequence, Koszul complex, depth）、10.5（homological dimension and regular rings）；这些内容能把 tensor/flat/Tor/projective dimension 的抽象语言压回到具体交换代数场景。
\end{itemize}
