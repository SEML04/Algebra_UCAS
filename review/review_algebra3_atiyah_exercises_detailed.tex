...
\textbf{证明。}
先证 \(\mathcal E\) 非空且有极大元。\(\mathcal E\) 非空是因为 \(A\) 非 Noetherian。对 \(\mathcal E\) 中任意链
\[
\{\mathfrak a_\lambda\}_{\lambda\in\Lambda},
\]
考虑其并
\[
...
\]
每个 \(u_i\) 落在某个 \(\mathfrak a_{\lambda_i}\) 中，而链是全序的，故存在 \(\lambda_0\) 使
\[
\mathfrak a_{\lambda_i}\subseteq \mathfrak a_{\lambda_0}\qquad (1\le i\le r).
\]
于是 \(u_1,\dots,u_r\in \mathfrak a_{\lambda_0}\)，从而
\[
\mathfrak a=(u_1,\dots,u_r)\subseteq \mathfrak a_{\lambda_0}\subseteq \mathfrak a,
\]
故 \(\mathfrak a=\mathfrak a_{\lambda_0}\)，这与 \(\mathfrak a_{\lambda_0}\in\mathcal E\) 矛盾。因此 \(\mathfrak a\) 仍不有限生成，即 \(\mathfrak a\in\mathcal E\)。由 Zorn 引理，\(\mathcal E\) 有极大元。
...
由 \(\mathfrak a+(x)\) 有限生成，可取有限生成理想 \(\mathfrak a_0\subseteq \mathfrak a\)，使得
\[
\mathfrak a_0+(x)=\mathfrak a+(x).
\]
这是因为只要取 \(\mathfrak a+(x)\) 的一组有限生成元，其中落在 \(\mathfrak a\) 中的那些生成一个有限生成理想 \(\mathfrak a_0\)，就有上式。

下面证明关键分解
\[
\mathfrak a=\mathfrak a_0+x(\mathfrak a:x),
\qquad
(\mathfrak a:x):=\{t\in A:xt\in \mathfrak a\}.
\]
先证左边包含于右边。任取 \(u\in \mathfrak a\)。因为
\[
u\in \mathfrak a\subseteq \mathfrak a+(x)=\mathfrak a_0+(x),
\]
可写
\[
...
\[
u\in \mathfrak a_0+x(\mathfrak a:x).
\]
反向包含显然成立，因为 \(\mathfrak a_0\subseteq \mathfrak a\)，且若 \(t\in(\mathfrak a:x)\)，则 \(xt\in \mathfrak a\)。于是得到所需等式。
...
现在 \(\mathfrak a_0\) 有限生成，\((\mathfrak a:x)\) 有限生成，因此
\[
x(\mathfrak a:x)
\]
也有限生成。由
...
\textbf{证明。}
先证必要性。若 \(f\) 幂零，则对任意素理想 \(\mathfrak p\subset A\)，其像 \(\bar f\) 在
\[
(A/\mathfrak p)[[x]]
\]
中也幂零。由于 \(A/\mathfrak p\) 是整环，而整环上的形式幂级数环仍是整环，所以幂零元只能是零元。于是 \(\bar f=0\)，即每个系数都落在 \(\mathfrak p\) 中：
\[
a_n\in \mathfrak p\qquad (\forall n\ge 0).
\]
...
\textbf{证明。}
\((i)\Rightarrow(ii)\)\; 由 \textit{Algebra III} 中的 Artinian 判据，即 (III.Thm 5.3.8)，对 Noetherian 环有
\[
A \text{ Artinian}\Longleftrightarrow \dim A=0.
\]
因此若 \(A\) Artinian，则每个素理想都是极大理想。再由 (III.Prop 5.3.5)，Artinian 环只有有限多个极大理想，所以 \(\operatorname{Spec}(A)\) 是有限集。

又因为在维数 \(0\) 的情形下每个素理想都是极大理想，也就是闭点，所以 \(\operatorname{Spec}(A)\) 是由有限个闭点组成的空间。有限空间中若每个点都闭，则每个单点集合都是开集，因此拓扑离散。故得到 (ii)。

\((ii)\Rightarrow(iii)\)\; 这是显然的。

\((iii)\Rightarrow(ii)\)\; 关键在于 Noetherian 环的谱空间是 Noetherian 拓扑空间，因此准紧。一个离散拓扑空间若准紧，则只能有有限多个点；否则由所有单点开的覆盖无法抽取有限子覆盖。故 \(\operatorname{Spec}(A)\) 离散时自动有限，于是 (ii) 成立。

\((ii)\Rightarrow(i)\)\; 若 \(\operatorname{Spec}(A)\) 离散且有限，则特别每个素理想都是闭点。可是在谱空间里闭点恰对应极大理想，因此所有素理想都是极大理想，即
\[
\dim A=0.
\]
...
\textbf{证明。}
\((ii)\Rightarrow(i)\)\; 若 \(A\) 是有限 \(k\)-代数，即 \(A\) 作为 \(k\)-向量空间有限维，则 \(A\) 的每个理想都是有限维 \(k\)-子空间。有限维向量空间满足降链条件，所以理想降链也稳定。因此 \(A\) 是 Artinian。

\((i)\Rightarrow(ii)\)\; 设 \(A\) Artinian。由 \textit{Algebra III} 中 Artinian 环的结构定理 (III.Thm 5.3.16)，
\[
A\cong A_1\times \cdots \times A_r
\]
其中每个 \(A_i\) 是 Artin 局部环。只要证明每个 \(A_i\) 都是有限 \(k\)-代数，便知它们的有限直积 \(A\) 也是有限 \(k\)-代数。因此可化归到局部情形。

设 \((A,\mathfrak m)\) 是 Artin 局部环。首先，由 Nullstellensatz 在 \textit{Algebra III} 中的形式，即 (III.Cor 4.3.2)，因为 \(A\) 是有限生成 \(k\)-代数且 \(A/\mathfrak m\) 是域，所以剩余域
\[
\kappa:=A/\mathfrak m
\]
是 \(k\) 的有限扩张。换言之，\(\kappa\) 作为 \(k\)-向量空间有限维。
...
现在利用 \(A\) Noetherian。因为 Artinian 环也是 Noetherian，可由 (III.Thm 5.3.8) 得到，所以每个理想 \(\mathfrak m^i\) 都有限生成为 \(A\)-模，故商模 \(\mathfrak m^i/\mathfrak m^{i+1}\) 也是有限生成 \(A\)-模。被 \(\mathfrak m\) 消去之后，这个有限生成 \(A\)-模实际上就是有限维 \(\kappa\)-向量空间。

于是 \(A\) 作为 \(\kappa\)-向量空间有一个有限长度滤过，其各层都是有限维 \(\kappa\)-向量空间，因此 \(A\) 本身也是有限维 \(\kappa\)-向量空间。再结合 \(\kappa\) 对 \(k\) 有限维，得到 \(A\) 对 \(k\) 也有限维，即 \(A\) 是有限 \(k\)-代数。
