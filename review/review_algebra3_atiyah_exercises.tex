...
\item 设 \(A\) 是 Dedekind 域，\(S\subset A\) 乘法闭。证明 \(S^{-1}A\) 不是 Dedekind 域就是 \(A\) 的分式域；并证明类群诱导满射。

\textbf{解答.}
若 \(S\) 含有非零素理想中的元素，则这些素理想在局部化后消失。若所有非零素理想都与 \(S\) 相交，则局部化得到分式域。否则剩下的非零素理想仍都是极大理想，局部环仍是 DVR，故 \(S^{-1}A\) 仍是 Dedekind 域。

任意 \(S^{-1}A\) 的非零分式理想都来自 \(A\) 的分式理想局部化，因此类群映射
\[
\operatorname{Cl}(A)\to \operatorname{Cl}(S^{-1}A)
\]
满射。
...
\textbf{解答.}
对每个极大理想 \(\mathfrak m\) 局部化。\(A_{\mathfrak m}\) 是 DVR，因此每个有限生成理想都是主理想。设
\[
...
\]
其中 \(f_0,g_0\) 至少有一个系数是单位，因此 \(c(f_0)=c(g_0)=A_{\mathfrak m}\)。故
\[
c(fg)A_{\mathfrak m}=(\pi^{r+s})=c(f)A_{\mathfrak m}\, c(g)A_{\mathfrak m}.
\]
对所有 \(\mathfrak m\) 成立，故全局成立。
